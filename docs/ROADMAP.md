...
- **加权策略** — 每题可配置不同权重
- **维度策略** — 按维度分组分别计分
- 维度均分自动转换（总分÷题数→均分，匹配 score range）
- 最严重维度颜色提升（取总分+所有维度中最高严重度 color）
- 反向题自动处理
- 分数→等级/颜色/建议自动匹配
- 量表缓存加速重复计算
- 单元测试覆盖率 >95%
- E2E 黑盒测试验证（SCL-90 30名学生，覆盖 green/yellow/red 三段）

### 测评任务
- 创建测评任务（关联量表 + 目标班级 + 截止时间）
- 任务状态管理（草稿 → 已发布 → 已完成），带状态机校验
- 按角色过滤任务列表：教师按创建者、学生按班级+已发布、管理员全量
- 学生在线答题（findOne 加载 scale 关系，正确渲染题目）
- 提交后自动计分并保存结果（事务包装，防重复提交）
- 结果为红/黄色时自动触发预警
- 已提交状态查询接口（GET /tasks/:id/submission-status）
- JWT 身份提取：createdById/studentId 从服务端 JWT 获取，杜绝冒充
